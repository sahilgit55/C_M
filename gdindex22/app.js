	...
	try {
		...
		for (i in files) {
			...
			if (item['mimeType'] == 'application/vnd.google-apps.folder') {
				...
				? item.thumbnailLink.replace(/=s\d+$/, '=s80') 
				: null;
				...
				if ("|mp4|MP4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${ext}|`) >= 0) {
				    html += thumb 
				        ? `<a href="${p.replace(/\/$/, '')}?a=view" target="_blank"><img src="${thumb}" class="file-thumb" style="width:80px; height:auto; margin-right:6px;"></a>` 
				        : video_icon;

...
function append_files_to_fallback_list(path, files) {
    ...
    ? item.thumbnailLink.replace(/=s\d+$/, '=s320')
    : 'https://cdn.jsdelivr.net/npm/@googledrive/index@2.2.3/images/poster.jpg';

				var ext = item.fileExtension
				//if ("|html|php|css|go|java|js|json|txt|sh|md|mp4|MP4|webm|avi|bmp|jpg|JPG|jpeg|png|gif|m4a|mp3|flac|wav|ogg|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|pdf|".indexOf(`|${ext}|`) >= 0) {
				...
				if ("|mp4|MP4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${ext}|`) >= 0) {
				    html += thumb 
				        ? `<a href="${p.replace(/\/$/, '')}?a=view" target="_blank"><img src="${thumb}" class="file-thumb" style="width:320px; height:auto; margin-right:6px;"></a>` 
				        : video_icon;
				} else if ("|html|php|css|go|java|js|json|txt|sh|".indexOf(`|${ext}|`) >= 0) {
				    html += code_icon;
				} else if ("|zip|rar|tar|.7z|.gz|".indexOf(`|${ext}|`) >= 0) {
				    html += zip_icon;
				} else if ("|bmp|jpg|JPG|jpeg|png|gif|".indexOf(`|${ext}|`) >= 0) {
				    html += thumb 
				        ? `<a href="${p.replace(/\/$/, '')}?a=view" target="_blank"><img src="${thumb}" class="file-thumb" style="width:320px; height:auto; margin-right:6px;"></a>` 
				        : image_icon;
				...
				html += ` <a class="countitems size_items list-group-item-action" style="text-decoration: none; color: ${UI.css_a_tag_color};" href="${p}&a=view" target="_blank"></a>${UI.display_download ? `<a href="${link}"><svg class="float-end"width="25px" style="margin-left: 8px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path> </svg></a>` : ``}${UI.display_size ? `<span class="badge bg-primary float-end"> ` + item['modifiedTime'] + ` </span>` : ``}${UI.display_time ? ` <a href="${p.replace(/\/$/, '')}?a=view" target="_blank">
   <span class="badge bg-info float-end">${item['name']}</span>
 </a>` : ``}</div>`;
			}
		}
...
function append_files_to_list(path, files) {
    ...
    : 'https://cdn.jsdelivr.net/npm/@googledrive/index@2.2.3/images/poster.jpg';
      ...
      if ("|mp4|MP4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${ext}|`) >= 0) {
			...
			html += ` <a class="countitems size_items list-group-item-action" style="text-decoration: none; color: ${UI.css_a_tag_color};" href="${pn}" target="_blank"></a>${UI.display_download ? `<a href="${link}"><svg class="float-end"width="25px" style="margin-left: 8px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path> </svg></a>` : ``}${UI.display_size ? `<span class="badge bg-primary float-end"> ` + item['modifiedTime'] + ` </span>` : ``}${UI.display_time ? `<a href="${p.replace(/\/$/, '')}?a=view" target="_blank">
   <span class="badge bg-info float-end">${item['name']}</span>
 </a>` : ``}</div>`;
		}
	}
...
if ("|mp4|MP4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${ext}|`) >= 0) {
				...
				html += ` <span onclick="onSearchResultItemClick('${item['id']}', true)" data-bs-toggle="modal" data-bs-target="#SearchModel"></span>${UI.display_download ? `<a href="${link}"><svg class="float-end"width="25px" style="margin-left: 8px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path> </svg></a>` : ``}<span class="badge float-end csize"> ${UI.display_size ? `<span class="badge bg-primary float-end"> ` + item['modifiedTime'] + ` </span>` : ``}${UI.display_time ? `<a href="${p.replace(/\/$/, '')}?a=view" target="_blank">
   <span class="badge bg-info float-end">${item['name']}</span>
 </a>` : ``}</div>`;
	...
	if (type) { // is a file id
		...
		fetch("/0:fallback", {
			...
			.then(function(obj) {
				...
				const video = ["mp4","MP4", "webm", "avi", "mpg", "mpeg", "mkv", "rm", "rmvb", "mov", "wmv", "asf", "ts", "flv", "3gp", "m4v"];
				const audio = ["mp3", "flac", "wav", "ogg", "m4a", "aac", "wma", "alac"];
				const image = ["bmp", "jpg", "JPG", "jpeg", "png", "gif", "svg", "tiff", "ico"];
				const pdf = ["pdf"];
				if (mimeType === "application/vnd.google-apps.folder") {
					...
					const time = utc2delhi(obj.modifiedTime);
					...
					if (mimeType.includes("video") || video.includes(fileExtension)) {
						...
						file_video(name, encoded_name, size, poster, url, mimeType, file_id, cookie_folder_id, time);
						...
						const poster = obj.thumbnailLink ? obj.thumbnailLink.replace("s220", "s0") : UI.poster;
						file_image(name, encoded_name, size, url, file_id, cookie_folder_id, poster, time);
...
// Document display video |mp4|MP4|webm|avi|
function file_video(name, encoded_name, size, poster, url, mimeType, file_id, cookie_folder_id, time) {
    ...
    <div class="container text-center"><br>
      ...
      <div class="card text-center">
        <div class="text-center">
          <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${name}<br>${size}<br>${time}</div>${player}</div>
        </br>
...
// image display
function file_image(name, encoded_name, size, url, file_id, cookie_folder_id, poster, time) {
	// Split the file path into parts
	var path = window.location.pathname;
	var pathParts = path.split('/');
	const copyFileBox = UI.allow_file_copy ? generateCopyFileBox(file_id, cookie_folder_id) : '';
    ...
    <div class="container text-center"><br>
      ...
      <div class="card">
        <div class="card-body text-center">
          <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${name}<br>${size}<br>${time}</div>
          <img src="${poster.replace(/=s\d+$/, '=s1024')}" id="load_image" height="100%" width="100%">
        </div>
        <div class="card-body">
          <div class="input-group mb-4">
